...
The underlying connector library used by Sequelize for Oracle is the [oracledb](https://www.npmjs.com/package/oracledb) package.  
See [Releases](/releases#oracle-support-table) to see which versions of Oracle Database & oracledb are supported.

`oracledb` needs [instant client library files](https://www.oracle.com/in/database/technologies/instant-client/downloads.html) to work. Installation process can be found on link mentioned above.
