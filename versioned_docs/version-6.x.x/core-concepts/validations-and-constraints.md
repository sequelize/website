...
To create the `username` field in the table as `` `username` TEXT UNIQUE``, [synchronize the model](./model-basics.md), for example by calling `sequelize.sync`. An attempt to insert a username that already exists will throw a `SequelizeUniqueConstraintError`.
